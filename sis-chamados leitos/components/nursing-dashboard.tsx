...
function CallCard({ 
  ...
  return (
    <Card className={cn(
      ...
      <CardContent className="p-1 sm:p-2 flex flex-col h-full w-full justify-between">
        {/* Top Row: Room Info and Timer */}
        <div className="flex items-start justify-between gap-1 sm:gap-1.5 mb-0.5 shrink-0">
          <div className="flex items-center gap-1.5 sm:gap-2 flex-1 min-w-0">
            <div className={cn("p-1 sm:p-1.5 rounded-lg shrink-0", priorityConfig.className)}>
              <Icon className="h-5 w-5 sm:h-6 sm:w-6" />
            </div>
            <div className="flex-1 min-w-0 flex items-center">
              <h3 className="text-lg sm:text-xl font-black text-foreground tracking-tight leading-none break-words whitespace-normal line-clamp-2" title={call.room || call.patientName || 'Quarto'}>
        ...
        {/* Middle Row: Details */}
        <div className="flex items-end justify-between gap-1 sm:gap-2 mb-0.5 w-full overflow-hidden flex-1 -mt-1">
          <div className="flex flex-col gap-0.5 flex-1 min-w-0">
            ...
            {call.ward && (
              <p className="text-[9px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-widest break-words whitespace-normal line-clamp-1">{call.ward}</p>
            )}
          </div>
        ...
        <div className="flex flex-col gap-1 pt-1 border-t-2 border-border/60 w-full mt-auto shrink-0">
          {/* Status indicator */}
          ...
          <div className="flex items-center gap-1 w-full mt-auto">
            {call.status === "pending" && (
              <>
                <Button
                  ...
                  className="h-6 sm:h-7 flex-1 px-1.5 text-[9px] sm:text-xs font-bold border-2"
                >
                  <Eye className="h-3 w-3 mr-1 shrink-0 hidden sm:block" />
                  <span className="truncate">Visualizar</span>
                </Button>
                <Button
                  size="sm"
                  onClick={() => onAttend(call.id)}
                  className="h-6 sm:h-7 flex-1 px-1.5 text-[9px] sm:text-xs font-bold bg-primary text-primary-foreground hover:bg-primary/90"
                >
                  <Play className="h-3 w-3 mr-1 shrink-0 hidden sm:block" />
            ...
            {call.status === "seen" && (
              <Button
                size="sm"
                onClick={() => onAttend(call.id)}
                className="h-6 sm:h-7 w-full px-2 sm:px-4 text-[10px] sm:text-xs font-bold bg-primary text-primary-foreground hover:bg-primary/90"
              >
                <Play className="h-3 w-3 mr-1 sm:mr-1.5 shrink-0" />
                Atender
              </Button>
            )}
            {call.status === "attending" && (
              <Button
                ...
                className="h-6 sm:h-7 w-full px-2 sm:px-4 text-[10px] sm:text-xs font-bold bg-success text-success-foreground hover:bg-success/90"
              >
                <CheckCircle2 className="h-3 w-3 mr-1 sm:mr-1.5 shrink-0" />
                Finalizar
              </Button>
            )}
          </div>
